...
def shit_on(bot, config):
    ...
    @bot.message_handler(func=lambda message: message.reply_to_message is not None and message.text.split()[0].lstrip('.').lower() in config['call_list16'])
    def handler(message):
        ...
        reply = f"{caller} обдристал {replied_user}. Еще если бы глаза витикали было бы харошо."
